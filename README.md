...
- Запускаем команду `gulp` - запустится процесс сборки. Результат будет доступен в браузере по адресу <http://localhost:3000/> (все доступные для просмотра адреса выводятся в консоль).
...
- В сборке используется обработка пост-процессором `PostCSS`. Описание плагинов, включенных в сборку:
  ...
  - `postcss-sort-media-queries` - сортирует респонсив медиа-выражения, объединяя повторяющиеся в одно объявление. В опциях плагина нужно указать, какой подход к верстке используется - mobile-first или mobile-first (по умолчанию)
